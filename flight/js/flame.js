...
function Flame(options = {}) {
  ...
  this.rotationIncrement = options['rotationIncrement'] || .125 * Math.PI;

  this.scaleRate = options['scaleRate'] || 1.02;
...
Flame.prototype = {
  ...
  prepare: function() {
    ...
    this.x = dragon.x + jitterX + dragon.width / 2 + 10;
    this.y = dragon.y + jitterY;

    // Base flame direction on dragon rotation.
    this.direction = dragon.rotation;
  ...
  update: function() {
    ...
    // Update flame scale
    this.width = this.width * this.scaleRate;
    this.height = this.height * this.scaleRate;
